...
const KNOWN_NOTES: Partial<Record<string, string>> = {
  ...
  ayuba:
    ...
    "Name's Abjad sum.",
  ali:
    "Computed Abjad of سالم (131) does not match the manuscript's stated count (370). The Hatim cells for Ali, " +
    "corrected to their authoritative manuscript values (366/365/364, Prompt 35), do agree with each other under " +
    'the N-4/N-5/N-6 formula (see hatimPattern.ts — status "confirmed_by_source", N=370) and match the stated ' +
    "recitation count exactly — the same relationship found for Ayuba and Iddris. The brief still flags Ali's " +
    "Divine Name as needing verification against the exact manuscript spelling for the Abjad calculation itself; " +
    "with no source image available for that spelling, it is reported as unresolved rather than inferred from the " +
    "transliteration — a separate question from the Hatim cell values, which are settled.",
  yunus:
...
export function buildAbjadValidations(
  ...
  return entries
    ...
    .map((e) => {
      ...
      const partialMatchWord =
        computedValue === statedValue
          ? null
          : findPartialMatch(divineNameOnly, statedValue);
      const status: AbjadStatus =
        computedValue === statedValue
          ? "match"
          : partialMatchWord
            ? "partial_match"
...
export const ABJAD_VALIDATIONS: AbjadValidation[] =
  buildAbjadValidations(STAR_USE_ENTRIES);

export function getAbjadValidationByStarId(
  starId: string,
): AbjadValidation | undefined {
