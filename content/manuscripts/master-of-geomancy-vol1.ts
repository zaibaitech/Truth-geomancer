...
/** The book's opening dedication page (source page 1), read before the
 * Introduction. Not a chapter — no chapter number, no chapter heading
 * styling; see the reader page for its distinct "opening page" treatment. */
export const DEDICATION_TITLE = "Dedication";

/** The Introduction's own heading as printed on source page 2 — carries no
 * chapter number, and must not be relabelled "Chapter 1". */
...
// Corrected against the source page's own wording: parenthetical names
// (not em-dash asides), "Jannatul Fridaus" as printed (not "Firdaus"), and
// the source's own sentence breaks preserved rather than run together.
export const DEDICATION =
  "My profound gratitude goes to my sheikh, mentor and guardian (Sheikh Alhaji Seidu Kassim Watara) and my parents (Ustaz Adam Meffe Lajina) May his gentle soul rest in perfect peace. May almighty Allah grant him Jannatul Fridaus. And my mother, who is still alive, may almighty Allah grant her long life to see Her grand children’s children.";
