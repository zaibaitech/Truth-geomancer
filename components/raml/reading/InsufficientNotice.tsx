...
import { sourceSilentConditionLine } from '@/lib/raml/resultPresentation';
import {
 ...
 *
 * `sourceSilent` is the Prompt 61 distinction: every verified method ran
 * and returned `uncertain` (the source does not determine this chart),
 * which is not the same as a method that failed to compute. */
export function InsufficientNotice({
  shortSummary,
  methods,
  sourceSilent = false,
}: {
  ...
  const heading = sourceSilent ? SOURCE_SILENT_HEADING : INSUFFICIENT_HEADING;
  const explanation = sourceSilent ? sourceSilentExplanation(verifiedCount) : INSUFFICIENT_EXPLANATION;

  return (
    <Card>
      {/* Prompt 15, section 14: the same state the engine calls
          `insufficient_data`, said in words that make clear the limit is the
          manuscript's rather than a failure of the app. Source-silent
          readings keep that internal state but use distinct copy. */}
      <p className="type-method font-semibold text-clay-light">{heading}</p>
      <p className="mt-1.5 type-body text-sand/75">{explanation}</p>
      {sourceSilent ? null : <p className="mt-1.5 type-body text-sand/70">{shortSummary}</p>}
