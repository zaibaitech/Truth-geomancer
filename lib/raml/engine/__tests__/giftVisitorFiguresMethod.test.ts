...
//
// PROMPT 50 CORRECTION: four fixtures (#2/#3/#5/#7) were regenerated by
// the same brute-force methodology, re-run against the corrected star
// targets (iddris/issah/ali/mahadi), after direct visual re-verification
// of the manuscript page found those four entries' original patterns had
// their third row misread (see content/manuscripts/giftVisitorFigures.ts's
// header comment). The "no gift recovered" example was also swapped: it
// used to be Iddris (then one of the eight NOT recovered); Iddris is now
// recovered (#2), so Musah — recovered under the old, incorrect mapping,
// NOT recovered under the corrected one — takes its place, which doubles
// as a direct regression check on the correction itself.
...
const FIXTURES: {
  ...
  {
    name: 'gift #2 (Iddris), not found',
    mothers: [[1, 1, 2, 1], [1, 1, 2, 1], [2, 1, 2, 2], [2, 1, 1, 1]],
    resultFigureId: 'iddris',
    found: false,
    giftNumber: 2,
  },
  {
    name: 'gift #3 (Issah), not found',
    mothers: [[1, 1, 2, 1], [1, 1, 2, 1], [1, 2, 2, 2], [2, 1, 1, 1]],
    resultFigureId: 'issah',
    found: false,
    giftNumber: 3,
  },
  {
  ...
  {
    name: 'gift #5 (Ali), not found',
    mothers: [[1, 1, 2, 1], [1, 1, 2, 1], [2, 2, 1, 2], [2, 1, 1, 1]],
    resultFigureId: 'ali',
    found: false,
    giftNumber: 5,
  },
  ...
  {
    name: 'gift #7 (Mahadi), not found',
    mothers: [[1, 1, 2, 1], [1, 1, 2, 1], [2, 2, 1, 2], [1, 1, 2, 1]],
    resultFigureId: 'mahadi',
    found: false,
    giftNumber: 7,
  },
  {
  ...
  {
    name: 'no gift recovered (Musah), found — one of the eight NOT recovered figures (displaced by the Prompt 50 correction)',
    mothers: [[1, 1, 2, 1], [1, 1, 2, 1], [2, 2, 2, 1], [2, 1, 1, 1]],
