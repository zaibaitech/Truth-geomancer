...
## Prompt 50 — Gift/Visitor figure mapping correction (four of eight patterns)

**The request.** New primary-source evidence (a direct visual
re-verification of the Gift/Visitor manuscript page, following the Prompt
49 reconciliation audit) found that four of the eight gift-figure patterns
originally restored above were misread. Correct only the affected figure-
to-meaning mapping and its dependent tests/documentation — not the
calculation, not the descriptive-result architecture, not entitlement.

**What was wrong.** Four of the eight recovered patterns had their third
row (of four) misread — every disagreement between the original mapping
and the re-verified one differs in exactly that one line, always 2 → 1,
never anywhere else:

| Ordinal | Was (incorrect) | Now (re-verified) |
|---|---|---|
| #2 | `2222` → Musah | `2212` → Iddris |
| #3 | `1222` → Adam | `1212` → Issah |
| #5 | `2122` → Umar | `2112` → Ali |
| #7 | `2121` → Usman | `2111` → Mahadi |

Entries #1 (Ibrahim), #4 (Hassan & Hussein), #6 (Nuhu), and #8 (Yunus)
were already correct and are unchanged. The interpretation TEXT attached
to each ordinal number (transcribed from `kanzulMikban.ts`'s own prose)
does not change — only which canonical star each ordinal number's pattern
resolves to. The eight canonical figures with a recovered Gift/Visitor
meaning are now Ibrahim, Iddris, Issah, Hassan & Hussein, Ali, Nuhu,
Mahadi, Yunus — the eight WITHOUT one are now Yussif, Adam, Umar, Ayuba,
Kalla Allahu, Sulemana, Usman, Musah.

**What changed.**
- `content/manuscripts/giftVisitorFigures.ts` — `GIFT_VISITOR_FIGURES`'s
  `starId` updated for ordinals #2/#3/#5/#7 (musah→iddris, adam→issah,
  umar→ali, usman→mahadi); header comment rewritten to document the
  correction and its provenance.
- `content/manuscripts/giftVisitorFigures.test.ts` — `EXPECTED` table
  updated for the same four ordinals; the "eight NOT recovered" assertion
  updated to the new displaced set; the `findGiftVisitorFigureByPattern`
  null-check example switched from Iddris (now recovered) to Musah (now
  unrecovered).
- `lib/raml/engine/__tests__/giftVisitorFiguresMethod.test.ts` — the four
  affected fixtures regenerated by the same exhaustive 16⁴ brute-force
  methodology already used for this file, re-targeted at the corrected
  canonical figures (Iddris/Issah/Ali/Mahadi), and cross-checked directly
  against the real engine (`buildChart`/`buildChartModel`/`EXTRACT_ELEMENT`/
  `ADD_FIGURE_TO_HOUSE`) before being written in; the "no gift recovered"
  example fixture switched from Iddris (now recovered) to Musah (now
  unrecovered, and itself a direct regression check on the correction).
- `lib/raml/engine/questions/giftVisitorFigures.ts` — header comment only
  (points at this correction); `GIFT_INTERPRETATIONS` (keyed by ordinal
  number, not by star) and the `calculate`/`evaluate` logic are unchanged.

**Not changed, confirmed by inspection.** The calculation
(`EXTRACT_ELEMENT`/`ADD_FIGURE_TO_HOUSE`/H5-H7-H11-H14/H7), the descriptive
result architecture, `moneyOrGoodStrangers.ts`, `casting.ts`,
`chartModel.ts`, `ruleEngine.ts`, `lib/server/content/kanzulMikban.ts`'s
prose, `lib/raml/questionAvailability.ts`, `lib/raml/questionRegistryMeta.ts`,
registry/method/verified counts (still 142/233/184), and the previously-
removed fire/air/water/sand elaboration (still absent, not reintroduced).

**Source-access limitation, stated plainly (unchanged from Prompt 49).**
This session was not able to independently view the manuscript image
itself at any point in Prompt 49 or Prompt 50 — no image file was
reachable in either session's environment. The correction was implemented
on the strength of the requester's explicit statement that direct visual
re-verification had been performed outside this session, the same
evidentiary channel that supplied the original (partly incorrect) mapping.
This is recorded here rather than glossed over.
