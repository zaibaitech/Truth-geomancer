  ...
  return (
    <div>
      ...
      <div className="px-4 py-4">
        ...
        {!available ? (
          <p className="mb-4 rounded-xl border border-clay/25 bg-clay/5 px-3 py-2.5 text-[12px] leading-relaxed text-sand/60">
        ...
        {entries === null ? null : entries.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-sand/10 py-10 text-center">
            ...
            <p className="text-sm font-medium text-sand-light">Your readings will appear here.</p>
            <p className="max-w-[260px] text-[12.5px] leading-relaxed text-sand/50">
          ...
          <>
                  ...
                  {FILTERS.map((f) => (
                    <button
                      ...
                      className={`shrink-0 rounded-full px-3 py-1.5 text-[11.5px] font-medium ${
                        filter === f.id ? 'bg-clay text-ink' : 'border border-sand/15 text-sand/60'
                      }`}
                    >
                      {f.label}
                    </button>
            ...
            <p role="status" className="mb-2 text-[11px] uppercase tracking-widest text-sand/40">
            ...
            <button
              ...
              className={`mt-6 flex w-full items-center justify-center gap-2 rounded-xl border py-2.5 text-sm ${
                confirmingClear ? 'border-clay/50 bg-clay/10 text-clay-light' : 'border-sand/15 text-sand/60'
              }`}
            >
              <Trash2 size={14} />
            ...
            {confirmingClear ? (
              <button
                ...
                className="mt-2 w-full rounded-xl py-2 text-xs text-sand/45"
              >
                Cancel
              </button>
            ) : null}
